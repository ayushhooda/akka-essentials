...
class SynchronousTestingSpec extends WordSpecLike with BeforeAndAfterAll {
  ...
  "A counter" should {
    "synchronously increase its counter" in {
      ...
      counter ! Inc // here counter has already received the message
    ...
    "work on the calling thread dispatcher" in {
      val counter = system.actorOf(Props[Counter].withDispatcher(CallingThreadDispatcher.Id)) // this will allow synchronous testing
      val probe = TestProbe()
      probe.send(counter, Read)
      probe.expectMsg(Duration.Zero, 0) // probe has already received 0
    }
  }
}
...
/*
Recap
1. Synchronous testing: all messages are handled in calling thread
  a. Using TestActorRef
