...
object Routers extends App {
  ...
  class Master extends Actor {
    ...
    private val slaves = for(i <- 1 to 5) yield {
      val slave = context.actorOf(Props[Slave], s"slaves_$i")
      context.watch(slave)
  ...
  /**
    * #2 - a router actor with it's own children
    * POOL router
    */
    // 2.1 programmatically (in code)
  val poolMaster = system.actorOf(RoundRobinPool(5).props(Props[Slave]), "simplePoolMaster")
  // RoundRobinPool will create 5 actors of Slave
  for (i <- 1 to 10) {
    poolMaster ! s"[$i] Hello from the world"
  }
  // This do same thing which we achieved using Master Actor
  ...
  /**
    * Special Messages
    */
  groupMaster2 ! Broadcast("hello, everyone") // This will be sent to every single slave

  // PoisonPill and Kill are NOT routed
  // AddRoutee, Remove, Get handled only by the routing actor


  // Goal: Spread/Delegate messages in b/w N identical actors
